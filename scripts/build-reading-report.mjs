...
const parseSections = answer => {
  ...
  return headings.map((heading, index) => {
    ...
    return {
      ...
      technicalTitle: cleanField(heading[4]),
      answer: field(segment, '先讲结论：', ['转换四步：', '命理推演：', '主题闭包：', '加减与边界：', '独立结构数：', '证据登记台账', '证据登记']),
      reasoning: field(segment, '命理推演：', ['主题闭包：', '加减与边界：', '独立结构数：', '时间承接层：', '语义层级：', '技术状态：', '证据登记台账', '证据登记']),
      synthesis: field(segment, '主题闭包：', ['加减与边界：', '独立结构数：', '时间承接层：', '语义层级：', '技术状态：', '证据登记台账', '证据登记']),
      boundary: field(segment, '加减与边界：', ['独立结构数：', '时间承接层：', '语义层级：', '最小充分结论：', '技术状态：', '证据登记台账', '证据登记']),
    };
