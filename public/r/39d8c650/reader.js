...
const allowedIds = ['report','portrait','psyche','sheng','jiaren'];
...
function mount(data) {
  if(!Array.isArray(data.documents)||data.documents.length!==5||data.documents.some((doc,index)=>doc.id!==allowedIds[index]||typeof doc.html!=='string'||typeof doc.text!=='string')) throw new Error('invalid data');
...
$('unlock-form').addEventListener('submit',async event=>{
  ...
  try{
    let envelope;
    try{const response=await fetch('/r/39d8c650/content.enc.json',{cache:'no-store',signal:AbortSignal.timeout(20000)});if(!response.ok)throw new Error();envelope=await response.json();}catch{throw new Error('network');}
